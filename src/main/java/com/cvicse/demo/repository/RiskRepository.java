...
public interface RiskRepository extends CrudRepository<Risk, String> {
    ...
    @Query(nativeQuery = true, value="SELECT b.* FROM (SELECT 合同编号, MAX(认定日期) as 认定日期  FROM RISK GROUP BY 合同编号) a   JOIN ( SELECT * FROM RISK)  b on (a.合同编号=b.合同编号 AND a.认定日期=b.认定日期)")
    public Iterable<Risk> findLastestRisks();
}